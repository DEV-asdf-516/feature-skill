...
4. 같은 접근법 안의 코드 표현 선택은 DELEGATED 다. "정해져 있지 않다"는 사실만으로는 이슈가 아니다. 접근법이 갈리는(solution shape) 결정이 비어 있는 경우만 후보로 두되, 갈리는 접근법 둘과 영향 하나를 적을 수 없거나 직접 범위에 명백한 단일 precedent 가 있으면 후보가 아니다. 새 구조물을 도입하는 결정에 REUSE/EXTEND/NEW 탐색 근거 세 가지가 빠져 있으면 REUSE_DISCOVERY_GAP 후보다 — 근거가 있으면 그 판단을 재론하지 않는다.
