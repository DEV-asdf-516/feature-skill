...
# claude 쪽 pre_bash_guard.sh 와 동일한 커밋 통제를 codex 워커에게 적용.
# exit 2 = 도구 호출 차단, stderr가 codex에게 거부 사유로 전달됨.
# 검증자는 read-only 샌드박스라 이 훅에 걸릴 일이 없음.
#
...
# 워커 커밋/푸시 금지 (스킬 강제 규칙: 커밋은 사용자 지시 시 수정자)
if echo "$TOOL_INPUT_TEXT" | grep -qE 'git\s+(commit|push)'; then
  echo "차단: 워커는 커밋/푸시할 수 없습니다. 커밋은 사용자 지시 시 수정자가 수행합니다." >&2
  exit 2
fi
