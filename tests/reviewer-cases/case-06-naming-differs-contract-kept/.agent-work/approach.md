...
기존 `src/ClientService.java:L7-L9` 의 `findOrThrow` 를 그대로 호출한다(NotFoundException → 404 는 기존 핸들러가 처리). 새 예외 타입·null 반환 금지.
## 결정 2: 전화번호 마스킹 [REQUIRED]
기존 `src/MaskingUtil.java:L3-L6` 의 `maskPhone` 을 재사용한다. 마스킹 로직을 다시 쓰지 않는다.
