...
public class ClientServiceTest {
  ...
  private static ClientUpdateRequest request(String name, String phone) {
  ...
  @Test void update_savesUpdatedClient() {
    when(repo.findById(1L)).thenReturn(java.util.Optional.of(new Client(1L, "Kim", "01012345678")));
    Client after = service.update(1L, request("Lee", "01099998888"));
  ...
  @Test void update_rejectsBlankName() {
    when(repo.findById(1L)).thenReturn(java.util.Optional.of(new Client(1L, "Kim", "01012345678")));
    assertThrows(IllegalArgumentException.class, () -> service.update(1L, request("  ", "01099998888")));
    verify(repo, never()).save(any());
  }

  @Test void update_rejectsNonDigitPhone() {
    when(repo.findById(1L)).thenReturn(java.util.Optional.of(new Client(1L, "Kim", "01012345678")));
    assertThrows(IllegalArgumentException.class, () -> service.update(1L, request("Lee", "010-9999-8888")));
    verify(repo, never()).save(any());
  }

  @Test void update_unknownId_throwsNotFound() {
    when(repo.findById(9L)).thenReturn(java.util.Optional.empty());
    assertThrows(NotFoundException.class, () -> service.update(9L, request("Lee", "01099998888")));
  }
}
