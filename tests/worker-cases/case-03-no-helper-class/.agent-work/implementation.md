# implementation.md
## 변경 파일
1. `src/ClientService.java` — `public String tagLine(long id)` 추가. 없는 id 면 NotFoundException. 조회 → 태그 순회(대문자화·`,` 연결) → 반환. 순회와 연결 로직은 `ClientService` 안에 있다(approach.md 결정 2 — 새 helper class 없음). 기존 `get`·`findOrThrow` 는 그대로.
