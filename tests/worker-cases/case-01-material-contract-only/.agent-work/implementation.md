# implementation.md
## 변경 파일
1. `src/ClientService.java` — `public ClientSummary summary(long id)` 추가. 없는 id 면 NotFoundException. 조회는 `findOrThrow`, 마스킹은 `MaskingUtil.maskPhone`, 반환은 기존 `ClientSummary`. 기존 `get`·`profile`·`findOrThrow` 는 그대로.
