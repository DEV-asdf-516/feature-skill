...
`profile` 과 같이 `new ClientProfile(client.id(), client.name())` 을 직접 쓴다. 기존 `profile(id)` 를 호출해 재사용하지 않는다(참조는 구조이지 호출 대상이 아니다 — `profile` 은 발행 없는 조회 API 로 그대로 둔다). 변환 helper 없음.
...
## 결정 4: `publishProfile` 의 구조와 순서 [REQUIRED] — 선형 blueprint
lookup → transform → publish → return 의 네 줄이며 지역 변수는 `client`, `profile` 둘뿐이다. 문서에 없는 분기(null 검사·조건부 발행)·fallback(발행 실패 시 대체 동작)·try/catch·재시도·중간 helper·발행 전후 로깅을 추가하지 않는다. 순서를 바꾸지 않는다(발행 전에 반환하거나, 변환 전에 발행할 수 없다).
```
public ClientProfile publishProfile(long id) {
  Client client = findOrThrow(id);
  ClientProfile profile = new ClientProfile(client.id(), client.name());
