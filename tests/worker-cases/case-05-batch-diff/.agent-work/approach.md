...
변경 감지는 `src/Client.java:L11-L15` 의 `fields()` 와 `src/ClientUpdate.java:L9-L13` 의 `fields()` 로 만든 필드 Map 두 개를 `src/DiffUtil.java:L3-L10` 의 `diff` 에 넘겨 `Map<String, FieldChange>` 를 얻고(지역 변수 `changes`), 그 Map 을 `src/ChangeLogWriter.java:L7-L11` 의 `writeAll` 에 **한 번에** 넘긴다. 필드별로 값을 비교하거나(`equals`/`!=` 나열), 개별 `FieldChange`·`ChangeLog` 를 만들거나, `write(...)` 를 필드마다 부르거나, 변경 목록을 따로 모아 합치는 코드를 서비스에 두지 않는다(컨벤션 규칙 3 의 Map 기반 일괄 처리). 탐색: 같은 모듈에서 변경 감지는 `DiffUtil.diff`, 이력 기록은 `ChangeLogWriter` 뿐이다.
## 결정 3: 갱신 사본 [REQUIRED] — REUSE `Client.apply`
`src/Client.java:L16-L18` 의 `apply(ClientUpdate)` 로 갱신 사본을 만들고(지역 변수 `after`) `ClientRepository.save` 에 넘긴 뒤 그 사본을 반환한다.
## 결정 4: `update` 의 구조 [REQUIRED] — blueprint
지역 변수는 `before`(조회 결과), `changes`(diff 결과), `after`(갱신 사본) 셋뿐이다. private helper 없음.
```
