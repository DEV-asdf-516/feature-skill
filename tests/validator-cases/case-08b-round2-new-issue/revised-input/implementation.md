...
2. `src/ClientService.java` — `public ClientSummary summary(long id)` 추가.
3. `src/ClientController.java` 신규 — `GET /clients/{id}/summary` 매핑 `summary(long id)` 추가, `ClientService.summary` 반환값을 그대로 반환.
4. `src/test/ClientSummaryTest.java` 신규 — 아래 테스트.
## 테스트
- 존재하는 id → 200, id·name·maskedPhone 반환.
- 없는 id → 200, 빈 name/maskedPhone 반환.
## 테스트 setup
